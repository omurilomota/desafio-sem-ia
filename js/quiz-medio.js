// Quiz Médio - 10 questões intermediárias com 20 segundos por questão
const quizQuestions = [
    {
...
class QuizMedio {
    ...
    updateTimerDisplay() {
        ...
        const warningThreshold = Math.ceil(20 * 0.3);
    ...
    updateProgressBar() {
        ...
        // Cor da barra de progresso para médio
    ...
    showResults() {
        ...
        this.questionText.innerHTML = `
            <h2 style="color: #FF9800;">🎉 Quiz Médio Concluído!</h2>
    ...
    saveScore(score, totalQuestions, correct, wrong, timeElapsed) {
        const leaderboard = JSON.parse(localStorage.getItem('quiz-leaderboard-medio')) || [];
        const newScore = {
            score: score,
        ...
        localStorage.setItem('quiz-leaderboard-medio', JSON.stringify(leaderboard));
    }
    ...
    generateReportHTML(report) {
        const timePerformance = this.getTimePerformance(report.avgTimePerQuestion, 20);
        const recommendations = this.generateRecommendations(report.categoryAccuracy, report.avgTimePerQuestion, 20);
        
        return `
            <div class="learning-report">
                <div class="report-header">
                    <h2>🎓 Relatório de Aprendizagem - Nível Médio</h2>
